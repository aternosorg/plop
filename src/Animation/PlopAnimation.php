...
class PlopAnimation extends Animation
{
    ...
    public function getBlockCommands(Block $block, string $startIf, string $prefix): array
    {
        ...
        return [
            'execute ' . $startIf . ' run summon minecraft:block_display ~' . number_format($x,1) . ' ~' . number_format($y,1) . ' ~' . number_format($z,1) . ' {shadow_strength:0f,interpolation_duration:' . $this->animationDuration . ',Tags:' . json_encode($tags) . ',transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0.45f,0.45f,0.45f],scale:[0.1f,0.1f,0.1f]},block_state:{Name:' . json_encode($block->getName()) . ', Properties:' . json_encode($block->getState()) . '}}',
