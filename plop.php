...
$options = ["function" => "plop:example", "output" => null, "animation" => null, "placement" => null];
$arguments = ["input" => null];
$currentOption = null;
$currentArgument = 0;

if (count($argv) <= ARGUMENTS_START) {
    echo "Usage: php plop.php <input> [--function|-f <function>] [--output|-o <output>] [--animation|-a <animation>] [--placement|-p <placement>]" . PHP_EOL . PHP_EOL;
    echo "Options:" . PHP_EOL;
    echo "  --function, -f <function>    The Minecraft function name to generate, e.g. plop:example" . PHP_EOL;
    echo "  --output, -o <output>        The output file to save the generated function to, outputs to STDOUT if not set" . PHP_EOL;
